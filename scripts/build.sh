...
function build_kernel(){
	...
	(cd ${SDFUSE_DIR} && {
		DISABLE_MKIMG=1 KCFG=${TARGET_KERNEL_CONFIG} KERNEL_SRC=${TOP_DIR}/${KERNEL_SRC} ./build-kernel.sh ${TARGET_OSNAME}
	})
...
function clean_all(){
	...
	cd $TOP_DIR/${KERNEL_SRC} && make distclean && cd -
	cd $TOP_DIR/${FRIENDLYWRT_SRC} && make clean && cd -
...
function prepare_image_for_friendlyelec_eflasher(){
    ...
    local KERNEL_DIR=${TOP_DIR}/${KERNEL_SRC}
    (cd ${SDFUSE_DIR} && {
        ./tools/update_uboot_bin.sh ${UBOOT_DIR} ./${OS_DIR}
	    echo [sky] ${UBOOT_DIR} ${OS_DIR} ${BOOT_DIR}
		if [ $? -ne 0 ]; then
...
function build_sdimg(){
    ...
    local HAS_BUILD_KERN=`has_built_kernel ${TOP_DIR}/${KERNEL_SRC} ${SDFUSE_DIR}/out`
    local HAS_BUILD_KERN_MODULES=`has_built_kernel_modules ${TOP_DIR}/${KERNEL_SRC} ${SDFUSE_DIR}/out`
	...
	echo [sky]:${TARGET_IMAGE_DIRNAME} ${TOP_DIR}/${FRIENDLYWRT_SRC}/${FRIENDLYWRT_ROOTFS}
